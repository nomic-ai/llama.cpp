...
void ggml_vk_mul_mat_mat_q4_x(const std::vector<uint32_t>& spirv, uint32_t block_size, kp::Sequence& seq,
                          const std::shared_ptr<kp::Tensor>& inA,
                          const std::shared_ptr<kp::Tensor>& inB,
                          const std::shared_ptr<kp::Tensor>& out,
                          uint32_t inAOff, uint32_t inBOff, uint32_t outOff,
...
void ggml_vk_mul_mat_mat_q4_0(Args&&... args) {
    ...
    ggml_vk_mul_mat_mat_q4_x(spirv, 1/*We access blocks unaligned*/, std::forward<Args>(args)...);
}
                ...
                case GGML_OP_MUL_MAT:
                    {
                        ...
                        if (!ggml_is_transposed(src0)
                            ...
                            switch (src0t) {
                                case GGML_TYPE_F16:
                                case GGML_TYPE_F32:
                                    ggml_vk_mul_mat_mat_f16(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, ne01, nb01, nb02, ne11, ne12, nb11, nb12, ne0, ne1);
                                    break;
                                case GGML_TYPE_Q4_0:
                                    //ggml_vk_mul_mat_mat_q4_0(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, ne01, nb01, nb02, ne11, ne12, nb11, nb12, ne0, ne1);
